...
const CopyCodeAction = ({ name }: Pick<Icon, "name">) => {
  ...
  const handleCopy = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const content = await getIconContent(name, framework);
...
const OpenInV0Action = ({ name }: Pick<Icon, "name">) => {
  const { framework } = useFramework();
  const [state, setState] = useState<IconStatus>("idle");
  const isReactOnly = framework !== "react";

  const handleOpenInV0 = async (e: React.MouseEvent) => {
    e.stopPropagation();
    e.preventDefault();

    if (state !== "idle" || isReactOnly) return;
  ...
  return (
    <Tooltip>
      <TooltipTrigger
        aria-disabled={state !== "idle" || isReactOnly}
        aria-label={isReactOnly ? "Open in v0 (React only)" : "Open in v0"}
        className={cn(
          "supports-[corner-shape:squircle]:corner-squircle flex size-10 cursor-pointer items-center justify-center rounded-[14px] bg-neutral-200/20 transition-[background-color] duration-100 focus-within:-outline-offset-1 hover:bg-neutral-200 focus-visible:outline-1 focus-visible:outline-primary supports-[corner-shape:squircle]:rounded-[20px] dark:bg-neutral-800/20 dark:hover:bg-neutral-700",
          isReactOnly && "cursor-not-allowed opacity-50"
        )}
        data-busy={state !== "idle" ? "" : undefined}
